function [OptEC, PSNR, T, OptMapID] = EmbedProc2D(I, a, b, Sele, NL, Tlog, Payload)
%%
[A, B] = size(I);
maps = {};
map = cell(3);
...
for m = 1 : mapNum
    ...
    %     log = [log; [m ECMT EDMT]];
    if ECMT >= Payload - EC1 && EDMT < OptED
        OptEC = ECMT;
        OptED = EDMT;
        OptMapID = m;
    end
...
ec = mapECs(OptMapID);
ed = mapEDs(OptMapID);
ec = ec{1}; ec4 = flipud(rot90(ec'));
ed = ed{1}; ed4 = flipud(rot90(ed'));

optMapEC0 = zeros(256,256); optMapEC0(1,:) = 1;  optMapEC0(:,1) = 1;  optMapEC0(1:3,1:3) = ec4;
optMapED0 = ones(256,256)*2;optMapED0(1,:) = 3/2;optMapED0(:,1) = 3/2;optMapED0(1:3,1:3) = ed4;
...
for i = 1:floor((A-2)/a)
    for j = 1:floor((B-2)/b)
        ...
        if NL(i,j) < Tmax
            ...
            if TSele(NL(i,j)+1) == 1
                EC(i,j) = optMapEC0(dmax+1,dmin+1);
                ED(i,j) = optMapED0(dmax+1,dmin+1);
            elseif TSele(NL(i,j)+1) == 2
                EC(i,j) = optMapEC1(dmax+1,dmin+1);
                ED(i,j) = optMapED1(dmax+1,dmin+1);
            end
        end
    end
end
